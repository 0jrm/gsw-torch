...
def _call_reference_exact(func_name, *args):
    ...
    if len(args_np[0]) == 0:
        ...
        result_np = np.array([func(*[arg_np[i] for arg_np in args_np]) 
                              for i in range(len(args_np[0]))])
...
def _call_reference_exact_tuple(func_name, *args):
    ...
    if len(args_np[0]) == 0:
        # Handle empty arrays
        results_np = tuple(np.array([]) for _ in range(len(func(*[arg_np[0] for arg_np in args_np]))))
    else:
        results_list = [func(*[arg_np[i] for arg_np in args_np]) 
                        for i in range(len(args_np[0]))]
        # Transpose: convert list of tuples to tuple of arrays
        num_outputs = len(results_list[0]) if results_list else 0
        results_np = tuple(np.array([r[i] for r in results_list]) 
                          for i in range(num_outputs))
    
    # Convert back to tensors
    results = tuple(torch.as_tensor(r_np, dtype=torch.float64, device=device).reshape(original_shape)
...
def melting_ice_into_seawater(SA, CT, p, w_Ih, t_Ih):
    ...
    w_Ih_final = torch.where(
        below_freezing,
        (h_seawater_initial - h_freeze) / (h_ice - h_freeze),
        torch.zeros_like(w_Ih)
    )
...
def melting_ice_equilibrium_SA_CT_ratio(SA, p):
    ...
    """
    from .freezing import t_freezing, CT_freezing
...
def melting_seaice_into_seawater(SA, CT, p, w_seaice, SA_seaice, t_seaice):
    ...
    for _ in range(10):
        ...
        SA_brine = torch.clamp(SA_brine, min=SA_seaice, max=torch.tensor(120.0, dtype=SA_brine.dtype, device=SA_brine.device))
...
def gibbs_ice(nt, np_order, t, p):
    ...
    t1 = torch.complex(torch.tensor(t1_real, dtype=torch.float64, device=t.device),
                       torch.tensor(t1_imag, dtype=torch.float64, device=t.device))
    t2 = torch.complex(torch.tensor(t2_real, dtype=torch.float64, device=t.device),
                       torch.tensor(t2_imag, dtype=torch.float64, device=t.device))
    r1 = torch.complex(torch.tensor(r1_real, dtype=torch.float64, device=t.device),
                       torch.tensor(r1_imag, dtype=torch.float64, device=t.device))
    r20 = torch.complex(torch.tensor(r20_real, dtype=torch.float64, device=t.device),
                        torch.tensor(r20_imag, dtype=torch.float64, device=t.device))
    r21 = torch.complex(torch.tensor(r21_real, dtype=torch.float64, device=t.device),
                        torch.tensor(r21_imag, dtype=torch.float64, device=t.device))
    r22 = torch.complex(torch.tensor(r22_real, dtype=torch.float64, device=t.device),
    ...
    if nt == 0 and np_order == 0:
        ...
        g = r1 * (1.0 / (t1 - tau_complex) + 1.0 / (t1 + tau_complex) - 2.0 / t1) + \
            r2 * (1.0 / (t2 - tau_complex) + 1.0 / (t2 + tau_complex) - 2.0 / t2)
        ...
        # Invalid combination
        raise ValueError(f"Invalid derivative orders: nt={nt}, np_order={np_order}. "
                        f"Must be 0, 1, or 2 for each.")
