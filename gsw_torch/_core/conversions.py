...
def p_from_z(z, lat, geo_strf_dyn_height=0, sea_surface_geopotential=0):
    ...
    for iteration in range(2):
        ...
        f = enthalpy_sso_0 + gs * (z - 0.5 * gamma * (z * z)) - (geo_strf_dyn_height + sea_surface_geopotential)
        
        # Calculate derivative: df/dp = db2pa * specvol_SSO_0(p)
        # specvol_SSO_0 = specvol(SSO, 0, p)
        specvol_sso_0 = specvol(
            torch.full_like(p_old, SSO),
        ...
        if iteration == 0:
            p_mid = 0.5 * (p + p_old)
            specvol_sso_0_mid = specvol(
                torch.full_like(p_mid, SSO),
                torch.zeros_like(p_mid),
                p_mid
            )
    ...
    # Set invalid values
    p = torch.where(invalid_mask,
                    torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=z.device),
                    p)
...
def SA_from_SP(SP, p, lon, lat):
    ...
    SA = torch.where(invalid_saar, torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=SP.device), SA)
...
def SP_from_SA(SA, p, lon, lat):
    ...
    # Set invalid results to invalid value
    SP = torch.where(invalid_saar, torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=SA.device), SP)
...
def t_from_CT(SA, CT, p):
    ...
    t = pt0 - p * (8.65483913395442e-6 -
          s1 * 1.41636299744881e-6 -
          p * 7.38286467135737e-9 +
          pt0 * (-8.38241357039698e-6 +
...
_SA_ENTROPY_CORRECTION_CHEB_COEFFS = torch.tensor([
    1.128531330840626e+00,
    -1.655960363231078e+00,
    -2.035867938746822e+00,
...
def entropy_from_pt(SA, pt):
    ...
    Notes
    -----
    This is a pure PyTorch implementation. Converts potential temperature to
    Conservative Temperature and then calculates entropy:
    entropy_from_pt = entropy_from_CT(SA, CT_from_pt(SA, pt))
    
    Note: Entropy depends only on SA and CT (not pressure), so this conversion
    is straightforward once CT is obtained from pt.
    """
    CT = CT_from_pt(SA, pt)
    return entropy_from_CT(SA, CT)
...
def CT_from_entropy(SA, entropy):
    ...
    for no_iter in range(2):
        ...
        CT_avg = 0.5 * (CT + CT_old)
        entropy_calc_avg = entropy_from_CT(SA, CT_avg)
...
def CT_from_rho(rho, SA, p):
    ...
    """
    from ..density import rho as rho_func, specvol_alpha_beta, specvol
    ...
    ct = torch.where(invalid_mask, torch.tensor(float('nan'), dtype=torch.float64, device=ct.device), ct)
...
def CT_first_derivatives(SA, pt):
    ...
    g_sa_t_mod = (1187.3715515697959 + 
        ...
        y_pt * (-548.4580073635929 +
        y_pt * (592.4012338275047 +
        y_pt * (-274.2361238716608 + 49.9394019139016 * y_pt)))) +
        y_pt * (-258.3988055868252 +
        y_pt * (-90.2046337756875 + y_pt * 10.50720794170734))) +
        y_pt * (3520.125411988816 +
        y_pt * (-1351.605895580406 +
        y_pt * (731.4083582010072 +
        y_pt * (-216.60324087531103 + 25.56203650166196 * y_pt)))))
    ...
    g_sa_mod = (8645.36753595126 +
        x * (-7296.43987145382 +
        x * (8103.20462414788 +
        y_pt * (2175.341332000392 +
        y_pt * (-274.2290036817964 +
        y_pt * (197.4670779425016 +
        ...
        y_pt * (1760.062705994408 +
        y_pt * (-450.535298526802 +
        y_pt * (182.8520895502518 +
        y_pt * (-43.3206481750622 + 4.26033941694366 * y_pt))))))
...
def CT_first_derivatives_wrt_t_exact(SA, t, p):
    ...
    CT_SA, = torch.autograd.grad(CT, SA_grad, grad_outputs=torch.ones_like(CT), create_graph=False, retain_graph=True)
    CT_t, = torch.autograd.grad(CT, t_grad, grad_outputs=torch.ones_like(CT), create_graph=False, retain_graph=True)
    CT_p_dbar, = torch.autograd.grad(CT, p_grad, grad_outputs=torch.ones_like(CT), create_graph=False, retain_graph=False)
...
def entropy_second_derivatives(SA, CT):
    ...
    from the reference implementation.
    """
    from .gibbs_helpers import gibbs_100_zerop, gibbs_200_zerop, gibbs_110_zerop
...
def pt_first_derivatives(SA, CT):
    ...
    # Compute derivatives using autograd
    pt_SA, = torch.autograd.grad(pt, SA_grad, grad_outputs=torch.ones_like(pt), create_graph=False, retain_graph=True)
    pt_CT, = torch.autograd.grad(pt, CT_grad, grad_outputs=torch.ones_like(pt), create_graph=False, retain_graph=False)
...
def pt_second_derivatives(SA, CT):
    ...
    pt_SA, = torch.autograd.grad(pt, SA_grad, grad_outputs=torch.ones_like(pt), create_graph=True, retain_graph=True)
    pt_CT, = torch.autograd.grad(pt, CT_grad, grad_outputs=torch.ones_like(pt), create_graph=True, retain_graph=True)
    
    # Compute second derivatives
    pt_SA_SA, = torch.autograd.grad(pt_SA, SA_grad, grad_outputs=torch.ones_like(pt_SA), create_graph=False, retain_graph=True)
    pt_SA_CT, = torch.autograd.grad(pt_SA, CT_grad, grad_outputs=torch.ones_like(pt_SA), create_graph=False, retain_graph=True)
    pt_CT_CT, = torch.autograd.grad(pt_CT, CT_grad, grad_outputs=torch.ones_like(pt_CT), create_graph=False, retain_graph=False)
...
def SAAR(p, lon, lat):
    ...
    indz0 = torch.where(p_clamped >= P_REF[-1], torch.tensor(NZ - 2, dtype=torch.long, device=p.device), indz0)
    ...
    def add_mean(data_in):
        ...
        data_mean = torch.sum(data_in * valid_mask.float(), dim=1, keepdim=True) / torch.clamp(nmean, min=1.0)
    ...
    def add_barrier(data_in, lon_vals, lat_vals, lon_grid, lat_grid, dlong_grid, dlat_grid):
        ...
        r_query = (lon_vals - LONGS_PAN[k_query]) / (LONGS_PAN[k_query + 1] - LONGS_PAN[k_query] + 1e-10)
        ...
        for kk in range(4):
            ...
            r_corner = (lon_corner - LONGS_PAN[k_corner]) / (LONGS_PAN[k_corner + 1] - LONGS_PAN[k_corner] + 1e-10)
            lats_line_corner = LATS_PAN[k_corner] + r_corner * (LATS_PAN[k_corner + 1] - LATS_PAN[k_corner])
        ...
        for kk in range(4):
            ...
            data_out[:, kk] = torch.where(replace_mask, data_mean.expand_as(data_in[:, kk]), data_in[:, kk])
    ...
    if torch.any(panama_apply):
        saar_upper = add_barrier(
            saar_upper, lon_normalized, lat_flat,
            lon_grid_vals, lat_grid_vals, dlong, dlat
        )
    ...
    sa_upper = (
        (1.0 - s1) * (saar_upper[:, 0] + r1 * (saar_upper[:, 1] - saar_upper[:, 0])) +
        s1 * (saar_upper[:, 3] + r1 * (saar_upper[:, 2] - saar_upper[:, 3]))
    )
    ...
    if torch.any(panama_apply_lower):
        saar_lower = add_barrier(
            saar_lower, lon_normalized, lat_flat,
            lon_grid_vals, lat_grid_vals, dlong, dlat
        )
    ...
    sa_lower = (
        (1.0 - s1) * (saar_lower[:, 0] + r1 * (saar_lower[:, 1] - saar_lower[:, 0])) +
        s1 * (saar_lower[:, 3] + r1 * (saar_lower[:, 2] - saar_lower[:, 3]))
    )
    ...
    result = torch.where(torch.abs(result) >= GSW_ERROR_LIMIT, torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=p.device), result)
    
    # Set invalid inputs to invalid value
    result = torch.where(invalid_mask, torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=p.device), result)
...
def _hill_ratio_at_sp2(t):
    ...
    dsp_drtx = (a1 + (2.0 * a2 + (3.0 * a3 + (4.0 * a4 + 5.0 * a5 * rtx0) * rtx0) * rtx0) * rtx0 +
    ...
    dsp_drtx = (a1 + (2.0 * a2 + (3.0 * a3 + (4.0 * a4 + 5.0 * a5 * rtxm) * rtxm) * rtxm) * rtxm +
...
def _SP_from_C_pss78(C, t, p):
    ...
    rp = 1.0 + (p * (e1 + e2 * p + e3 * p * p)) / (1.0 + d1 * t68 + d2 * t68 * t68 + (d3 + d4 * t68) * R)
    ...
    SP = torch.where(invalid_mask | (SP < 0.0), torch.tensor(float('nan'), dtype=torch.float64, device=C.device), SP)
...
def _C_from_SP_iterative(SP, t, p):
    ...
    mask_p = SP >= 9.0
    if torch.any(mask_p):
        rtx_p = (p0 + x * (p1 + p4 * t68 + x * (p3 + p7 * t68 + x * (p6 + p11 * t68 + x * (p10 + p16 * t68 + x * p15)))) +
                 t68 * (p2 + t68 * (p5 + x * x * (p12 + x * p17) + p8 * x + t68 * (p9 + x * (p13 + x * p18) + t68 * (p14 + p19 * x + p20 * t68)))))
        rtx = torch.where(mask_p, rtx_p, rtx)
    
    # 0.25 <= SP < 9.0
    mask_q = (SP >= 0.25) & (SP < 9.0)
    if torch.any(mask_q):
        rtx_q = (q0 + x * (q1 + q4 * t68 + x * (q3 + q7 * t68 + x * (q6 + q11 * t68 + x * (q10 + q16 * t68 + x * q15)))) +
                 t68 * (q2 + t68 * (q5 + x * x * (q12 + x * q17) + q8 * x + t68 * (q9 + x * (q13 + x * q18) + t68 * (q14 + q19 * x + q20 * t68)))))
        rtx = torch.where(mask_q, rtx_q, rtx)
    
    # 0.003 <= SP < 0.25
    mask_s = (SP >= 0.003) & (SP < 0.25)
    if torch.any(mask_s):
        rtx_s = (s0 + x * (s1 + s4 * t68 + x * (s3 + s7 * t68 + x * (s6 + s11 * t68 + x * (s10 + s16 * t68 + x * s15)))) +
                 t68 * (s2 + t68 * (s5 + x * x * (s12 + x * s17) + s8 * x + t68 * (s9 + x * (s13 + x * s18) + t68 * (s14 + s19 * x + s20 * t68)))))
        rtx = torch.where(mask_s, rtx_s, rtx)
    
    # SP < 0.003
    mask_u = SP < 0.003
    if torch.any(mask_u):
        rtx_u = (u0 + x * (u1 + u4 * t68 + x * (u3 + u7 * t68 + x * (u6 + u11 * t68 + x * (u10 + u16 * t68 + x * u15)))) +
                 t68 * (u2 + t68 * (u5 + x * x * (u12 + x * u17) + u8 * x + t68 * (u9 + x * (u13 + x * u18) + t68 * (u14 + u19 * x + u20 * t68)))))
        rtx = torch.where(mask_u, rtx_u, rtx)
    
    # Calculate derivative dSP_dRtx
    dsp_drtx = (a1 + (2.0 * a2 + (3.0 * a3 + (4.0 * a4 + 5.0 * a5 * rtx) * rtx) * rtx) * rtx +
    ...
    if torch.any(low_SP_mask):
        ...
        dsp_drtx_hill = (dsp_drtx + a0 * 800.0 * rtx * (1.5 + 2.0 * x_hill) / (part1_hill * part1_hill) +
                        b0 * ft68 * (10.0 + sqrty_hill * (20.0 + 30.0 * sqrty_hill)) / (part2_hill * part2_hill))
    ...
    if torch.any(low_SP_mask):
        ...
        dsp_drtx_hill = (dsp_drtx + a0 * 800.0 * rtxm * (1.5 + 2.0 * x_hill) / (part1_hill * part1_hill) +
                        b0 * ft68 * (10.0 + sqrty_hill * (20.0 + 30.0 * sqrty_hill)) / (part2_hill * part2_hill))
        dsp_drtx_hill = hill_ratio * dsp_drtx_hill
...
def SP_from_SK(SK):
    ...
    SP = torch.where(invalid_mask, torch.tensor(float('nan'), dtype=torch.float64, device=SP.device), SP)
...
def SA_from_rho(rho, CT, p):
    ...
    # Set invalid values to NaN
    sa = torch.where(invalid_mask, torch.tensor(float('nan'), dtype=torch.float64, device=sa.device), sa)
