...
def frazil_properties(SA_bulk, h_bulk, p):
    ...
    from .ice import cp_ice, enthalpy_ice
    ...
    CT_final[no_ice_mask] = CT_from_enthalpy_exact(
        SA_bulk[no_ice_mask], h_bulk[no_ice_mask], p[no_ice_mask]
    )
    ...
    if torch.any(ice_mask):
        ...
        dfunc_dw_ih_mean_poly = (
            3.347814e05 - num_f * func0_ice * (1.0 + num_f2 * func0_ice) - num_p * p_ice
        )
        ...
        if torch.any(invalid_mask):
            SA_final[ice_mask] = torch.where(
                invalid_mask,
                torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=SA_bulk.device),
                SA_final[ice_mask],
            )
            CT_final[ice_mask] = torch.where(
                invalid_mask,
                torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=SA_bulk.device),
                CT_final[ice_mask],
            )
            w_Ih_final[ice_mask] = torch.where(
                invalid_mask,
                torch.tensor(GSW_INVALID_VALUE, dtype=torch.float64, device=SA_bulk.device),
                w_Ih_final[ice_mask],
            )
        ...
        dfunc_dw_ih = (
            hf - h_ihf - sa * (h_hat_sa + h_hat_ct * ctf_sa + w_ih * cp_ih * tf_sa / (1.0 - w_ih))
        )
        ...
        for _iteration in range(3):
            ...
            h_hat_sa_mean, h_hat_ct_mean = enthalpy_first_derivatives_CT_exact(
                sa_mean, ctf_mean, p_ice
            )
        ...
        if torch.any(negative_mask):
            ...
            ctf_final_ice[negative_mask] = CT_from_enthalpy_exact(
                SA_bulk_ice[negative_mask], h_bulk_ice[negative_mask], p_ice[negative_mask]
            )
...
def frazil_ratios_adiabatic(SA, p, w_Ih):
    ...
    from .ice import adiabatic_lapse_rate_ice, cp_ice, enthalpy_ice
