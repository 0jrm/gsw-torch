...
def dynamic_enthalpy(SA, CT, p):
    ...
    part_1 = (
        h001
        + ss * (h101 + ss * (h201 + ss * (h301 + ss * (h401 + ss * (h501 + h601 * ss)))))
        + tt
        * (
            h011
            + ss * (h111 + ss * (h211 + ss * (h311 + ss * (h411 + h511 * ss))))
            + tt
            * (
                h021
                + ss * (h121 + ss * (h221 + ss * (h321 + h421 * ss)))
                + tt
                * (
                    h031
                    + ss * (h131 + ss * (h231 + h331 * ss))
                    + tt * (h041 + ss * (h141 + h241 * ss) + tt * (h051 + h151 * ss + h061 * tt))
                )
            )
        )
    )
    ...
    part_2 = (
        h002
        + ss * (h102 + ss * (h202 + ss * (h302 + ss * (h402 + h502 * ss))))
        + tt
        * (
            h012
            + ss * (h112 + ss * (h212 + ss * (h312 + h412 * ss)))
            + tt
            * (
                h022
                + ss * (h122 + ss * (h222 + h322 * ss))
                + tt * (h032 + ss * (h132 + h232 * ss) + tt * (h042 + h142 * ss + h052 * tt))
            )
        )
    )
    ...
    part_3 = (
        h003
        + ss * (h103 + ss * (h203 + ss * (h303 + h403 * ss)))
        + tt
        * (
            h013
            + ss * (h113 + ss * (h213 + h313 * ss))
            + tt * (h023 + ss * (h123 + h223 * ss) + tt * (h033 + h133 * ss + h043 * tt))
        )
    )
    ...
    dynamic_enthalpy_part = z * (
        part_1 + z * (part_2 + z * (part_3 + z * (part_4 + z * (part_5 + z * (h006 + h007 * z)))))
    )
...
def enthalpy_first_derivatives(SA, CT, p):
    ...
    (h_SA,) = torch.autograd.grad(
        h, SA_grad, grad_outputs=torch.ones_like(h), create_graph=False, retain_graph=True
    )
    (h_CT,) = torch.autograd.grad(
        h, CT_grad, grad_outputs=torch.ones_like(h), create_graph=False, retain_graph=False
    )
...
def enthalpy_second_derivatives(SA, CT, p):
    ...
    (h_SA,) = torch.autograd.grad(
        h, SA_grad, grad_outputs=torch.ones_like(h), create_graph=True, retain_graph=True
    )
    (h_CT,) = torch.autograd.grad(
        h, CT_grad, grad_outputs=torch.ones_like(h), create_graph=True, retain_graph=True
    )
    ...
    (h_SA_SA,) = torch.autograd.grad(
        h_SA, SA_grad, grad_outputs=torch.ones_like(h_SA), create_graph=False, retain_graph=True
    )

    # h_SA_CT = d²h/(dSA dCT) = d(h_SA)/dCT
    (h_SA_CT,) = torch.autograd.grad(
        h_SA, CT_grad, grad_outputs=torch.ones_like(h_SA), create_graph=False, retain_graph=True
    )

    # h_CT_CT = d²h/dCT² = d(h_CT)/dCT
    (h_CT_CT,) = torch.autograd.grad(
        h_CT, CT_grad, grad_outputs=torch.ones_like(h_CT), create_graph=False, retain_graph=False
    )
