...
def _sa_ct_interp_core(sa, ct, pgood, pi):
    ...
    for i in range(prof_len):
        ct_f = CT_freezing_poly(
            sa[i : i + 1], p_obs[i : i + 1], torch.zeros(1, dtype=torch.float64, device=sa.device)
        )
    ...
    for k in range(7):
        ...
        ct_i_obs_plus_interp = ct_i_obs_plus_interp + (
            -q_i * sin_kpi_on_16[k] + v_i * cos_kpi_on_16[k]
        )
        sa_i_obs_plus_interp = sa_i_obs_plus_interp + rec_factor * (
            q_i * cos_kpi_on_16[k] + v_i * sin_kpi_on_16[k]
        )
    ...
    sa_i_limiting_obs_plus_interp, ct_i_limiting_obs_plus_interp = _linear_interp_sa_ct(
        ...
        prof_len,
        independent_variable_obs_plus_interp,
        i_obs_plus_interp_len,
    )

    # Freezing point corrections - iterative process
    p_obs_plus_tensor_for_freeze = torch.tensor(
        p_obs_plus_interp, dtype=torch.float64, device=sa.device
    )
    ...
    # Map to output
    sa_i_tooutput = torch.full(
        (i_surf_and_obs_plus_interp_len,), float("nan"), dtype=torch.float64, device=sa.device
    )
    ct_i_tooutput = torch.full(
        (i_surf_and_obs_plus_interp_len,), float("nan"), dtype=torch.float64, device=sa.device
    )

    if abs(min_p_obs) > 1e-10:
        # Handle surface extrapolation
        p_i_tmp_tensor = torch.tensor(
            [p_surf_and_obs_plus_interp[i] for i in range(i_surf_and_obs_plus_interp_len)],
            dtype=torch.float64,
            device=sa.device,
        )
...
def _tracer_ct_interp_core(tr, ct, pgood, pi, factor):
    ...
    for k in range(7):
        ...
        tracer_i_obs_plus_interp = tracer_i_obs_plus_interp + rec_factor * (
            q_i * cos_kpi_on_16[k] + v_i * sin_kpi_on_16[k]
        )
    ...
    # Map to output
    tracer_i_tooutput = torch.full(
        (i_surf_and_obs_plus_interp_len,), float("nan"), dtype=torch.float64, device=tr.device
    )
    ct_i_tooutput = torch.full(
        (i_surf_and_obs_plus_interp_len,), float("nan"), dtype=torch.float64, device=tr.device
    )
