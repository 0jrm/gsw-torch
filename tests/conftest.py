...
def check_values():
    ...
    """
    cv_path = os.path.join(os.path.dirname(__file__), "../reference/gsw/tests/gsw_cv_v3_0.npz")
...
def assert_torch_allclose(actual, expected, rtol=1e-6, atol=1e-8, equal_nan=True):
    ...
    if not torch.allclose(actual, expected, rtol=rtol, atol=atol, equal_nan=equal_nan):
        ...
        raise AssertionError(f"Tensors not close: max_diff={max_diff}, max_rel_diff={max_rel_diff}")
